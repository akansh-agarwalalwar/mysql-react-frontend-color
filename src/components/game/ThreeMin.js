...
function TwoMin() {
  ...
  useEffect(() => {
    const fetchInitialPeriodAndTime = async () => {
      try {
        const periodResponse = await axios.get("http://api.perfectorse.site/period-timer/two-min");
        setPeriod(Number(periodResponse.data.periodNumber));

        const timeResponse = await axios.get("http://api.perfectorse.site/period-time/get-time/two-min");
  ...
  const fetchLastPeriodData = async () => {
    try {
      const response = await axios.get("http://api.perfectorse.site/winner-api/two-min");
  ...
  useEffect(() => {
    ...
    const sendTimeDataToServer = async () => {
      try {
        await axios.post("http://api.perfectorse.site/period-time/two-min", {
        ...
        if (time === 7) {
          await axios.post("http://api.perfectorse.site/update-status/two-min", {
            periodNumber: formatPeriod(period),
            periodDate: new Date().toISOString().split("T")[0],
          });
        }
  ...
  useEffect(() => {
    if (time === 10) {
      const updateAmounts = async () => {
        try {
          await axios.post("http://api.perfectorse.site/update-amounts/two-min", {
            periodNumber: formatPeriod(period),
          });
  ...
  const savePeriodToDatabase = async (newPeriod) => {
    try {
      await axios.post("http://api.perfectorse.site/period-timer/post/two-min", {
