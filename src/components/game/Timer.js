...
function Timer() {
  ...
  const fetchLastPeriodData = async () => {
    try {
      ...
      const response = await axios.get(
        "https://api.vigya.in/api/v1/user/winner-thirty-second"
      );
  ...
  const lastColorWinThirty = async () => {
    try {
      const response = await axios.get(
        `https://api.vigya.in/api/v1/user/lastColorWinThirty`
      );
  ...
  const handleBetPlacement = async (
    ...
    try {
      const response = await axios.post("https://api.vigya.in/place-bet", {
        userId: user.userId,
