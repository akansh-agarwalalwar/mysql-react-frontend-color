...
function PaymentApprove() {
  ...
  const fetchPayments = useCallback(async () => {
    try {
      const [pendingResponse, historyResponse] = await Promise.all([
        axios.get("https://api.perfectorse.ste/api/v1/admin/pendingPayment"),
        axios.get("https://api.perfectorse.ste/api/v1/admin/adminUpdatedHistory"),
      ]);
  ...
  const handleApprove = async (id) => {
    try {
      await axios.post(
        "https://api.perfectorse.ste/api/v1/admin/adminPaymentApprove",
        { id }
      );
  ...
  const handleDeny = async (id) => {
    try {
      await axios.post("https://api.perfectorse.ste/api/v1/admin/adminPaymentDeny", {
        id,
      });
      setPendingPayments(
        pendingPayments.filter((payment) => payment.id !== id)
