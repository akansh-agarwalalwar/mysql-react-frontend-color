...
export default function AllUsers() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("http://api.perfectorse.site/api/v1/admin/all-users");
  ...
  const fetchBankDetails = async (userId) => {
    try {
      const res = await fetch(`http://api.perfectorse.site/api/v1/admin/bank?userId=${userId}`);
      if (!res.ok) {
