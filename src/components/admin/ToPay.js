...
export default function ToPay() {
  ...
  const fetchWithdrawHistory = async () => {
    try {
      const response = await axios.get('http://65.2.75.197:3001/api/withdrawl/history');
  ...
  const fetchProcessedWithdrawHistory = async () => {
    try {
      const response = await axios.get('http://65.2.75.197:3001/api/withdrawl/processed-history');
  ...
  const handleAccept = async (id) => {
    try {
      await axios.post('http://65.2.75.197:3001/api/withdrawals/accept', { id });
  ...
  const handleDeny = async (id) => {
    try {
      await axios.post('http://65.2.75.197:3001/api/withdrawals/deny', { id });
