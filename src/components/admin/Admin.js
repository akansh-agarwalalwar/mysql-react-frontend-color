...
export default function AdminDashboard() {
  ...
  const fetchApproveUser = async () =>{
    try {
      const response = await fetch("https://color-server.onrender.com/api/payments/pending");
      const data = await response.json();
      setApproveUser(data);
    }
    catch(error){
  ...
  const fetchToPayUser = async () =>{
    try {
      const response = await fetch("https://color-server.onrender.com/api/withdrawl/history");
      const data = await response.json();
      setToPay(data);
    }
    catch(error){
