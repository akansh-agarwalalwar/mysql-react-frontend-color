...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch("http://65.2.75.197:3001/all-users");
  ...
  const fetchApproveUser = async () =>{
    try {
      const response = await fetch("http://65.2.75.197:3001/api/payments/pending");
      const data = await response.json();
      setApproveUser(data);
    }
    catch(error){
  ...
  const fetchToPayUser = async () =>{
    try {
      const response = await fetch("http://65.2.75.197:3001/api/withdrawl/history");
      const data = await response.json();
      setToPay(data);
    }
    catch(error){
