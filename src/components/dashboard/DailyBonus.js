...
function DailyBonus() {
  ...
  const handleClaimBonus = async () => {
    if (bonusEarned) {
      ...
      try {
        const response = await axios.post(`https://api.perfectorse.ste/api/v1/user/claim-bonus`, { userId: user?.userId });
  ...
  const checkIfUserHasRecharged = async () => {
    try {
      const response = await axios.get(`https://api.perfectorse.ste/api/v1/user/check-bonus/recharge-history/${user?.userId}`);
